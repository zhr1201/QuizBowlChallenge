...
class SpacyNERWIKITAGReranker(AbsReranker):
    def __init__(self, weight: float):
        ...
        super().__init__()
        self.weight = weight
        self.wiki_page_dict = defaultdict(dict)
        #self.answer_docs = {}
        #self.tokenizer = None
        #self.model = None
    ...
    def _rescore_one_page(
        ...
        if page_info[0] in self.wiki_page_dict:
            page = self.wiki_page_dict[page_info[0]]['ner']
        else:
    ...
    def train(
        ...
        for ans in tqdm(np.unique(answers).tolist()):
            try:
                self.wiki_page_dict[ans]['text'] = old_wiki_dict[ans]['text']#wikipedia.page(pageid=wiki_pageid)
            except:
                ...
                try:
                    ...
                    self.wiki_page_dict[ans]['text'] = wikipedia.page(pageid=wiki_pageid).content #
                except:
                    logger.warning("Using direct page search%s "  % ans)
                    try:
                        self.wiki_page_dict[ans]['text'] = wikipedia.page(ans, auto_suggest=False).content
                    except:
                        logger.warning("Using direct page search without space%s "  % ans)
                        try:
                            self.wiki_page_dict[ans]['text'] = wikipedia.page(re.sub(r'\s+','',ans), auto_suggest=False).content
                        except:
                            logger.warning("Fail to get " + ans)
        for ans in self.wiki_page_dict:
            entities = self.ner_tagger(self.wiki_page_dict[ans]['text']).ents
            self.wiki_page_dict[ans]['ner'] = []
            for ent in entities:
                self.wiki_page_dict[ans]['ner'].append([ent.text.lower(),ent.label_])
        #self.answer_docs = answer_docs       
    ...
    def load(
        ...
        Returns:
            the loaded model
        '''
        reranker = SpacyNERWIKITAGReranker(weight)
