...
from gensim.parsing.preprocessing import strip_tags, strip_punctuation, strip_multiple_whitespaces,remove_stopwords,stem_text,strip_short

class BM25PRERetriever(AbsRetriever):
	def __init__(self, k: int):
		...
		self.cumstom_filters = [lambda x: x.lower(), strip_tags, strip_punctuation, strip_multiple_whitespaces,remove_stopwords,stem_text,strip_tags, 
                  strip_punctuation, strip_multiple_whitespaces,remove_stopwords,stem_text,strip_short]
	...
	def train(self, path: str) -> None:
		...
		x_array = []
		y_array = []
		#x_array = preprocess_documents(list(answer_docs.values()),filters=self.cumstom_filters)
		#y_array = list(answer_docs.keys())
		for ans, doc in answer_docs.items():
			tokens = preprocess_string(doc,filters=self.cumstom_filters)
			# remove stop words
			x_array.append(tokens)
			y_array.append(ans)
	...
	def retrieve(self, questions: List[str]) -> List[List[Tuple[str, float]]]:
		...
		for question in questions:
			tokens = preprocess_string(question,filters=self.cumstom_filters)
			# remove stop words
			x_array.append(tokens)
	...
	@classmethod
	def load(cls, path: str, k: int) -> 'BM25PRERetriever':
		'''
		Args:
