...
class SpacyNERNNQEReranker(AbsReranker):
    def __init__(self,):
        '''
        Args:
    ...
    def _rescore_one_page(
        ...
        if page_info[0] in self.answer_docs:
            page = self.answer_docs[page_info[0]]
        else:
            page =''
    ...
    def train(
        ...
        try:
            ...
            for i in range(len(questions)):
                ...
                for j in range(len(top_k)):
                    ...
                    x[-2] = guess_score[i][j]
                    if top_k[j] in self.answer_docs:
                        for ner in ner_tags:
                            #try:
                            x[tags_to_id[ner[1]]] += self.answer_docs[top_k[j]].count(ner[0])
        ...
        #         if ans in self.answer_docs:
        #             if ans==answers[top_k]:
        #                 pages_with_tag.append([self.answer_docs[ans],1])
        #             else:
        #                 pages_with_tag.append([self.answer_docs[ans],0])
