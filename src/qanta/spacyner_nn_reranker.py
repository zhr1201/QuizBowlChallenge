...
class SpacyNERNNReranker(AbsReranker):
    def __init__(self,):
        '''
        Args:
    ...
    def _rerank_one_question(
        ...
        ret = []
        entities = self.ner_tagger(question).ents
        for result in top_k:
    ...
    def _rescore_one_page(
        ...
        for entity in entities:
            x[self.tags_to_id[entity.label_]] += page.count(entity.text)
    ...
    def train(
        ...
        try:
            ...
            for i in range(len(questions)):
                top_k = guess_pages[i]
                ner_tags = self.ner_tagger(questions[i]).ents
                for j in range(len(top_k)):
                    ...
                    if top_k[j] in self.wiki_page_dict:
                        for ner in ner_tags:
                            #try:
                            x[tags_to_id[ner.label_]] += self.wiki_page_dict[top_k[j]].count(ner.text)
        ...
        #clf = Perceptron(tol=1e-5, random_state=0,n_jobs=24,verbose=1)
