...
@cli.command()
@click.option('--host', default='2.100.17.0')
...
if __name__ == '__main__':
	...
	questions  = dataset.training_data()[0]
	answers = dataset.training_data()[1]
	correct = 0
	for i in range(len(questions)):
		result = tfidf_guesser.guess(questions[i], 1)
		if answers[i] == result[-1][0]:
			correct += 1
			
	training_accuracy = correct/len(questions)
	print("training accuracy: ", training_accuracy)
	
	
	# test accuracy
	print("Evaluating test accuracy...")
	questions  = dataset.test_data()[0]
	...
	test_accuracy = correct/len(questions)
